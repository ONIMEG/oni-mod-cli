use crate::cli_dialogue::{
    create_csproj_with_name, create_csproj_without_name, create_sln_with_name,
...
fn main() {
    ...
    match &cli.command {
        Commands::Create { create_commands } => match create_commands {
            CreateCommands::Sln { name } => handel_create_sln(name),
            CreateCommands::Csproj { name } => handel_create_csproj(name),
        },
    }
    clean_lockfile();
}
